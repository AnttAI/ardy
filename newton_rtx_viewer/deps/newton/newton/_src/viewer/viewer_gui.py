...
class ViewerGui:
    ...
    def _apply_camera_orbit_drag(self, dx: float, dy: float, modifiers: int = 0) -> bool:
        """Apply orbit-style mouse navigation to the active camera."""
        import pyglet
    ...
    def handle_mouse_drag(
        ...
        """Handle mouse drag: left free-looks, middle orbits, Shift pans, Ctrl dollies, right picks."""
        import pyglet
        ...
        if buttons & pyglet.window.mouse.MIDDLE and camera is not None:
            self._apply_camera_orbit_drag(dx, dy, modifiers)
            return
