    ...
    wp::array_t<wp::uint8> var_dst)
{
         ...
         _idx += static_cast<size_t>(blockDim.x) * static_cast<size_t>(gridDim.x))
    {
        ...
        //---------
        // forward
        // def copy_vec4ub_to_bgra(src: wp.array2d(dtype=wp.vec4ub), dst: wp.array3d(dtype=wp.uint8)):       <L 1341>
        // x, y = wp.tid()                                                                        <L 1342>
    ...
    wp::array_t<wp::uint8> adj_dst)
{
         ...
         _idx += static_cast<size_t>(blockDim.x) * static_cast<size_t>(gridDim.x))
    {
        ...
        // adj: dst[y, x, 3] = wp.uint8(255)                                                      <L 1347>
        ...
        // adj: dst[y, x, 0] = rgba[2]                                                            <L 1344>
        wp::adj_copy(var_4, adj_2, adj_3);
        wp::adj_address(var_src, var_1, var_0, adj_src, adj_1, adj_0, adj_2);
        // adj: rgba = src[y, x]                                                                  <L 1343>
        // adj: x, y = wp.tid()                                                                   <L 1342>
        // adj: def copy_vec4ub_to_bgra(src: wp.array2d(dtype=wp.vec4ub), dst: wp.array3d(dtype=wp.uint8)):  <L 1341>
        continue;
    }
}
    ...
    wp::array_t<wp::uint8> var_buf)
{
         ...
         _idx += static_cast<size_t>(blockDim.x) * static_cast<size_t>(gridDim.x))
    {
        ...
        //---------
        // forward
        // def swap_rb(buf: wp.array3d(dtype=wp.uint8)):                                          <L 1328>
        // x, y = wp.tid()                                                                        <L 1329>
        builtin_tid2d(var_0, var_1);
        // r = buf[y, x, 0]                                                                       <L 1330>
