...
extern "C" __global__ void compute_mesh_mesh_edge_counts_a604ae33_cuda_kernel_forward(
    ...
    wp::array_t<wp::int32> var_edge_counts)
{
         ...
         _idx += static_cast<size_t>(blockDim.x) * static_cast<size_t>(gridDim.x))
    {
        ...
        //---------
        // forward
        // def compute_mesh_mesh_edge_counts(                                                     <L 803>
        // i = wp.tid()                                                                           <L 818>
        var_0 = builtin_tid1d();
        // pair_count = wp.min(shape_pairs_mesh_mesh_count[0], shape_pairs_mesh_mesh.shape[0])       <L 819>
        ...
        // if i >= pair_count:                                                                    <L 820>
        var_9 = (var_0 >= var_7);
        if (var_9) {
            // edge_counts[i] = 0                                                                 <L 821>
            wp::array_store(var_edge_counts, var_0, var_10);
            // return                                                                             <L 822>
            continue;
        }
        // pair_encoded = shape_pairs_mesh_mesh[i]                                                <L 824>
        var_11 = wp::address(var_shape_pairs_mesh_mesh, var_0);
        var_13 = wp::load(var_11);
        var_12 = wp::copy(var_13);
        // has_hfield = (pair_encoded[0] & SHAPE_PAIR_HFIELD_BIT) != 0                            <L 825>
        var_15 = wp::extract(var_12, var_14);
        var_17 = wp::bit_and(var_15, var_16);
        var_19 = (var_17 != var_18);
        // pair = wp.vec2i(pair_encoded[0] & SHAPE_PAIR_INDEX_MASK, pair_encoded[1])              <L 826>
        ...
        // pair_edges = int(0)                                                                    <L 827>
        var_28 = wp::int(var_27);
        // for mode in range(2):                                                                  <L 828>
        // is_hfield = has_hfield and mode == 0                                                   <L 829>
        var_30 = var_19;
        if (var_30) {
            var_32 = (var_29 == var_31);
            var_30 = var_30 && var_32;
        }
        // shape_idx = pair[mode]                                                                 <L 830>
        var_33 = wp::extract(var_26, var_29);
        // if is_hfield:                                                                          <L 831>
        if (var_30) {
            // hfd = heightfield_data[shape_heightfield_index[shape_idx]]                         <L 832>
        ...
        // edge_counts[i] = wp.int32(pair_edges)                                                  <L 836>
...
extern "C" __global__ void compute_block_counts_from_weights_89c44be4_cuda_kernel_forward(
    wp::launch_bounds_t<1> dim,
    ...
    wp::array_t<wp::int32> var_block_counts)
{
         ...
         _idx += static_cast<size_t>(blockDim.x) * static_cast<size_t>(gridDim.x))
    {
        ...
        //---------
        // forward
        // def compute_block_counts_from_weights(                                                 <L 840>
        // i = wp.tid()                                                                           <L 855>
        var_0 = builtin_tid1d();
        // pair_count = wp.min(pair_count_arr[0], max_pairs)                                      <L 856>
        var_2 = wp::address(var_pair_count_arr, var_1);
        var_4 = wp::load(var_2);
        var_3 = wp::min(var_4, var_max_pairs);
        // if i >= pair_count:                                                                    <L 857>
        var_5 = (var_0 >= var_3);
        if (var_5) {
            // block_counts[i] = 0                                                                <L 858>
            wp::array_store(var_block_counts, var_0, var_6);
            // return                                                                             <L 859>
            continue;
        }
        // total_weight = weight_prefix_sums[pair_count - 1]                                      <L 862>
        var_8 = wp::sub(var_3, var_7);
        var_9 = wp::address(var_weight_prefix_sums, var_8);
        var_11 = wp::load(var_9);
        var_10 = wp::copy(var_11);
        // weight_per_block = int(total_weight)                                                   <L 863>
        var_12 = wp::int(var_10);
        // if target_blocks > 0 and total_weight > 0:                                             <L 864>
        ...
        // block_counts[i] = wp.int32(blocks)                                                     <L 872>
