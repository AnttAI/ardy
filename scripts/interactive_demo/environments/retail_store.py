"""Simple retail-store environment built from Viser primitives."""
...
RETAIL_RACKS = (
    ...
    RackSpec("rack_4", (0.00, 0.0, -3.43), yaw_rad=-np.pi / 2.0),
)


def add_retail_store_environment(client: viser.ClientHandle) -> list[viser.SceneHandle]:
    """Add the retail-store racks, counter, and boundary to a client scene."""
    handles: list[viser.SceneHandle] = [_add_work_area_boundary(client)]
    for rack in RETAIL_RACKS:
...
def _add_rack(client: viser.ClientHandle, rack: RackSpec) -> list[viser.SceneHandle]:
    root = f"/retail_store/racks/{rack.name}"
        ...
        if index <= len(rack.products):
            handles.append(
                client.scene.add_label(
                    ...
                    position=np.array([0.0, surface_height + 0.08, RACK_DEPTH_M / 2.0 + 0.03]),
...
def _add_products(client: viser.ClientHandle, rack: RackSpec) -> list[viser.SceneHandle]:
    handles: list[viser.SceneHandle] = []
    rack_center = np.asarray(rack.position, dtype=np.float64)
    cos_yaw = np.cos(rack.yaw_rad)
    sin_yaw = np.sin(rack.yaw_rad)
    for shelf_number, shelf_height in enumerate(_shelf_surface_heights(), start=1):
        for object_index in range(1, 4):
            local_x = RACK_WIDTH_M / 2.0 - 0.05
            local_z = (object_index - 2) * 0.15
            position = np.array(
                [
                    rack_center[0] + cos_yaw * local_x + sin_yaw * local_z,
                    rack_center[1] + shelf_height + PRODUCT_SIZE_M / 2.0,
                    rack_center[2] - sin_yaw * local_x + cos_yaw * local_z,
                ],
                dtype=np.float64,
            )
            handles.append(
                client.scene.add_box(
                    f"/retail_store/products/{rack.name}/shelf_{shelf_number}/object_{object_index}",
                    dimensions=(PRODUCT_SIZE_M,) * 3,
...
def _add_counter(client: viser.ClientHandle) -> list[viser.SceneHandle]:
    root = "/retail_store/counter"
    center_z = COUNTER_DISTANCE_FROM_ORIGIN_M + COUNTER_WIDTH_M / 2.0
    handles: list[viser.SceneHandle] = [
        client.scene.add_frame(
            root,
            show_axes=False,
    ...
    top_center_y = COUNTER_HEIGHT_M - COUNTER_TOP_THICKNESS_M / 2.0
    ...
    handles.append(
        client.scene.add_box(
            f"{root}/front_panel",
            dimensions=(COUNTER_LENGTH_M, leg_height * 0.70, 0.025),
            color=(116, 78, 50),
...
def _add_work_area_boundary(client: viser.ClientHandle) -> viser.SceneHandle:
    ...
    return client.scene.add_line_segments(
        "/retail_store/work_area_boundary",
        points=points,
